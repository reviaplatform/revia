...
export const getSystemInstruction = (customer: ICustomerDB, deviceData?: IDeviceDB | null): string => {
  const platform = deviceData?.platform?.toString().toLowerCase() || '';
  const deviceType = ['ios', 'android'].includes(platform) ? 'mobile'
    : ['windows', 'macos', 'linux'].includes(platform) ? 'laptop'
      : '';
  const formattedPlatform = platform === 'other' ? 'Other' : (deviceData?.platform?.toString() || 'Not provided');

  return `You are the Revia Assistant, a specialized diagnostic agent for mobile devices and laptops.
...
### PRE-CHAT CONTEXT
${deviceData ? `The user has already provided the following device details:
- Customer Name: ${customer.name}
- Device Type: ${deviceType || 'Not provided'}
- Manufacturer: ${deviceData.manufacturer}
- Device Model: ${deviceData.deviceModel}
- Platform: ${formattedPlatform}
- OS Version: ${deviceData.osVersion || 'Not provided'}

Acknowledge these details implicitly, do not ask for them again. Proceed directly to asking about the symptoms.` : 'No pre-chat device details provided.'}
...
export async function chatWithAssistant(
  ...
  const platform = deviceData?.platform?.toString().toLowerCase() || '';
  const laptopPlatforms = ['windows', 'macos', 'linux'];
  const mobilePlatforms = ['ios', 'android'];

  const deviceType = laptopPlatforms.includes(platform) ? 'laptop' : mobilePlatforms.includes(platform) ? 'mobile' : '';
  ...
  while (isToolCall) {
    response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: getSystemInstruction(customer, deviceData),
        temperature: 0.7,
        tools: [diagnosticTools as any, { googleSearch: {} }],
    ...
    if (response.functionCalls && response.functionCalls.length > 0) {
      if (onToolCall) {
        // Just report the first tool conceptually for the UI
        onToolCall(response.functionCalls[0].name);
      }
      ...
      for (const call of response.functionCalls) {
        if (call.name === 'search_repair_manuals') {
          ...
          const model = deviceData?.deviceModel || '';
  ...
  log.info(`[AI Chat] Response : ${JSON.stringify(response)}`);
  return response;
}
